...
class DeepSteg(nn.Module):
    def __init__(self, batch_size, im_dim):
        ...
        hnet = [nn.Conv2d(c_in, c_out, kernel_size=kernel, stride=1, padding=pad) \     # Convolution
                    for c_in, c_out, kernel, pad \
                    in zip(self._h_in_channels, self._h_out_channels, \
                        self._h_kernel_sizes, self._h_paddings)]
        h_batchnorms = [nn.BatchNorm2d(num_features=C) for C in self._h_out_channels]   # Batch Norm.
        ...
        self._r_out_channels = [50] * (self._r_layer_num - 1) + [self._image_channels,]
        self._r_kernel_sizes = [5,5,5,5,5]
        self._r_paddings = [kernel // 2 for kernel in self._r_kernel_sizes]
        rnet = [nn.Conv2d(c_in, c_out, kernel_size=kernel, stride=1, padding=pad) \     # Convolution
                for c_in, c_out, kernel, pad \
        ...
        r_nls = [nn.ReLU(),] * 4 + [nn.Sigmoid(),]                                      # Non-linearities

        self._rnet = nn.ModuleList(list(reduce(lambda a,b:a+b, zip(rnet, r_batchnorms, r_nls))))

    def forward(self, covers, secrets, device):
